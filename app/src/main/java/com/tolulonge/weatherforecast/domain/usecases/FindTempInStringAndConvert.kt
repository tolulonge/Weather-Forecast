...
/**
 * This use case searches for instances of temperature values with the string and modifies the values
 * to english words. e.g 28°C -> 28 degrees
 */
class FindTempInStringAndConvert(
    ...
    operator fun invoke(text: String?): String? {
        ...
        if (indexOfTempMatcher2 != -1) {
            ...
            resultingString =
                determineTempRangeInWordsForTempMatcher2(resultingString, tempRangeEnd)
        }
        return resultingString
    }

    private fun determineTempRangeInWordsForTempMatcher1(
        text: String,
        tempRangeStart: Int
    ): String {
        ...
        return if (text[i] == '°') {
            text.substring(0, tempRangeStart) + convertTempRangeToWords(
                minRange,
                maxRange
            ) + text.substring(i + 2)
        } else {
    ...
    private fun determineTempRangeInWordsForTempMatcher2(text: String, tempRangeEnd: Int): String {
        ...
        while (j > 0 && !text[j].isDigit()) {
            ...
            if (timeInLoop > 2) {
                return text.substring(
                    0,
                    j + 4
                ) + "${convertTemperatureValueToWords(String(maxRange).toInt())} degrees" + text.substring(
                    tempRangeEnd + 3
                )
            }
        }
        ...
        return text.substring(0, j + 1) + convertTempRangeToWords(
            String(minRange),
            String(maxRange)
        ) + text.substring(tempRangeEnd + 3)
    }
